...
For this purpose, the data frame object returned by the `co.co` function can be included in the `em.co_interventionalControlled` function. The following information must also be passed into the function:
...
The following optional arguments can be used to change the type of effect measure and adjust the calculation:
...
Afterwards, the effect measures can easily be pooled by using functions of other packages, e.g. the `metagen` function of the meta package by Dr. Guido Schwarzer.
